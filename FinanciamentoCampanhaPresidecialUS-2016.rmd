...
É importante realizarmos uma analise sobre a distribuição das contribuições entre os candidatos. Para uma melhor compreensão das doações entre os candidatos, agrupei o mesmo histograma de 2 formas: _a primeira, pela quantidade de doações e a segunda pelo valor total arrecadado._
...
hist_cand_ordenado <- function(campo, cond = NULL, titulo) {
  ...
  if (campo == "qtde"){
  g <- ggplot(aes(x = cand_nm), 
         data = df)+
    geom_bar(stat = 'count')+
    ylab("Qtde de contribuições")
  } else if (campo == "total") {
  g <- ggplot(aes(x = cand_nm, y = contb_receipt_amt), 
         data = df)+
    geom_bar(stat = 'summary', fun.y = 'sum')+
    ylab("Total de contribuições ($)")
  }
...
g +
  ...
  scale_x_log10(breaks=c(1, 10, 20, 100, 1000, 10000, 100000))
...
ggplot(aes(x = contb_receipt_dt, fill = contb_receipt_amt_range), 
  ...
  geom_histogram()+
  geom_vline(xintercept =  as.Date("2016-03-01"), linetype = 2, color = 'red')+
  scale_x_date(date_breaks = "4 months", 
               date_minor_breaks = "2 months", 
               date_labels = "%b/%Y",
               limits = as.Date(c("2015-01-01", "2016-12-31")))+
  ...
  scale_color_manual(labels = c("Primarias"), values = c("red"))+
  labs(fill = "Faixa de valor", color = "Marco")+
