...
candidatos_ordenados_por_qtde <- as.ordered(arrange(USACampaign.by_candidate, desc(qtde))$cand_nm)
candidatos_ordenados_por_valor_total <- arrange(USACampaign.by_candidate, desc(valor_total))$cand_nm
...
top7_cand <- head(candidatos_ordenados_por_qtde, 7)
...
Nesta analise percebemos que as principais cidades do estado então entre aquelas com mais números de contribuição.

```{r Distribuição das contribuições na 30 maiores cidades, echo = FALSE, warning = FALSE}

str(as.factor(USACampaign$city))
...
De forma semelhante à análise anterior, para o histograma por profissão do contribuinte considerei apenas as 10 com mais contribuições.

```{r Distribuicao por ocupação, echo=FALSE}

str(as.factor(USACampaign$contbr_occupation))

USACampaign.by_occupation <- subset(USACampaign, !is.na(contbr_occupation)) %>%
...
ggplot(aes(y=contb_receipt_amt, x=contbr_occupation), 
       data = subset(USACampaign, contbr_occupation %in% top10_occupation), outline = FALSE)+
  geom_boxplot()+
  ylim(0,125)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
A minha intenção neste trabalho é analisar como foi a distribuição de doações para os cnadidatos. Para esta análise, as principais atributos deste conjunto de dados são os candidatos (*cand_nm*), os valores das contribuições (*contb_receipt_amt*) e a data de contribuição (*contb_receipt_dt*).
...
Nesta analise percebemos que os valores doados se iniciaram principalmente 2 trimestre de 2015 com um média ligeiramente maior, se mantendo quase contantes a partir de agosto de 2015 até o final de 2016.
...
Na distribuição das doações entre as cidades do estado do TX, temos Austin em segundo, atrás apenas da cidade de Houston, porém quando analisamos o valor total, Dallas vem em segundo, deixando Austin em terceiro. Isto poderia estar relacionada a renda media de cada cidade mas sem os dados de rendimentos não é possível uma analise mais apurada.
...
ggplot(aes(x=contb_receipt_dt_month, y=contb_receipt_amt, fill=contb_receipt_amt_range), data = USACampaign)+
  geom_bar(stat = 'summary', fun.y = 'sum')+
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "bottom")+
  facet_grid(party~contb_receipt_dt_year)
...
ggplot(aes(y = contb_receipt_amt, x = contb_receipt_date, fill = contb_receipt_amt_range), 
       data=subset(USACampaign, cand_nm %in% head(candidatos_ordenados_por_valor_total, 7) & !is.na(contb_receipt_amt)))+
  ...
  facet_wrap(~cand_nm)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position="bottom")+
  ggtitle('Doações ao longo do tempo, por candidato')

ggplot(aes(x = contb_receipt_date, fill = contb_receipt_amt_range), 
       data=subset(USACampaign, cand_nm %in% head(candidatos_ordenados_por_qtde, 7) & !is.na(contb_receipt_amt)))+
...
O primeiro gráfico apresenta as doações ao longo do perídio de doação para cada candidato. Nele podemos notar algumas características interesantes. 
