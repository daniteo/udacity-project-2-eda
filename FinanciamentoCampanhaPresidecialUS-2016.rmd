...
```{r Distribuição das contribuições por valor 2, echo = FALSE}
ggplot(aes(x=contb_receipt_amt), 
       data = USACampaign)+
...
comite_ordered <- USACampaign.by_candidate$cand_nm

USACampaign$cand_nm <- factor(USACampaign$cand_nm, comite_ordered)
...
ggplot(aes(x = contb_receipt_date, y = contb_receipt_amt, color = cand_nm), 
       data = USACampaign)+
