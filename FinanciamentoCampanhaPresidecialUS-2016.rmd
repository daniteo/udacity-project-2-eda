...
USACampaign.by_candidate <- USACampaign %>%
                      group_by(cand_nm) %>%
                      summarise(count = n()) %>%
                      arrange(desc(count))

cand_ordered <- USACampaign.by_candidate$cand_nm

USACampaign$cand_nm <- factor(USACampaign$cand_nm, cand_ordered)

top5_cand <- head(USACampaign.by_candidate$cand_nm, 5)
...
Nesta analise podemos perceber a presença de faixas horizontais bem definidas para os valores doados com mais frequncias. Estes normalmente ão valores redodndos. Uma faixa bem definida é a 2700 dolares, que o limite máximo permitido para pessoas físicas.
