...
USACampaign$contb_receipt_dt <-  as.Date(USACampaign$contb_receipt_dt, "%d-%b-%y")
USACampaign$contb_receipt_dt_month <-  as.Date(USACampaign$contb_receipt_dt_month, "%d-%b-%y")
...
USACampaign$cand_nm <- factor(USACampaign$cand_nm, lista_ordenada("cand_nm", ordenado_por = "qtde")$cand_nm)
...
hist_cand_ordenado <- function(campo, cond = NULL, titulo) {
  ...
  USACampaign$cand_nm <- factor(USACampaign$cand_nm, lista_ordenada("cand_nm", ordenado_por = campo)$cand_nm)
...
hist_cand_ordenado("qtde", titulo = "Histograma das contribuições (ordenados por quantidade)")

#Histograma ordenado por valor total
hist_cand_ordenado("total", titulo = "Histograma das contribuições (ordenados por valor total)")

top7_cand <- as.ordered(lista_ordenada("cand_nm", ordenado_por = "qtde", quantidade =  7)$cand_nm)
hist_cand_ordenado("qtde", top7_cand, titulo = "Histograma das contribuições (Top 7)")
...
ggplot(aes(x = election_tp), data = subset(USACampaign, election_tp %in% c("G2016","P2016")))+
  geom_bar(stat = 'count')+
...
USACampaign$contb_receipt_amt_range <- cut(USACampaign$contb_receipt_amt, c(0,100,1000,2700,max(USACampaign$contb_receipt_amt)))
...
USACampaign$city_ordered <- factor(USACampaign$city, lista_ordenada("city", ordenado_por = "qtde")$city)

# Distribuição das contribuições entre as cidades com mais contribuições
ggplot(aes(x=city_ordered), 
       data = subset(USACampaign, city %in% lista_ordenada("city", 30, ordenado_por = "qtde")$city))+
  geom_bar(stat = 'count')+
...
De forma semelhante à análise anterior, para o histograma por profissão do contribuinte considerei apenas as 15 com mais contribuições.

```{r Distribuicao por ocupação, echo=FALSE, warning=FALSE}

str(as.factor(USACampaign$contbr_occupation))

lista_ordenada_ocupacao <- lista_ordenada("contbr_occupation", ordenado_por = "qtde")$contbr_occupation

# Ordena as cidades com base na quantidade de contribuições
USACampaign$contbr_occupation <- factor(USACampaign$contbr_occupation, lista_ordenada_ocupacao)

df_occup <- subset(USACampaign, !is.null(contbr_occupation) & !is.na(contbr_occupation) & contbr_occupation %in% head(lista_ordenada_ocupacao, 15))
...
ggplot(aes(y=contb_receipt_amt, x=contbr_occupation), 
       data = df_occup, outline = FALSE)+
  geom_boxplot()+
  ylim(0,125)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

rm(lista_ordenada_ocupacao)
...
g <- ggplot(aes(x = contb_receipt_dt_month, y = contb_receipt_amt), 
  ...
  geom_hline(yintercept = 2700, linetype = 2)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  ggtitle('Distribuição das contribuições no período')
...
ggplot(aes(x=cand_nm, fill = election_tp), 
       data=subset(USACampaign, cand_nm %in% lista_ordenada("cand_nm", 3, ordenado_por = "total")$cand_nm))+
  geom_bar(stat = 'count')
...
#texas@data$color <- ifelse(texas@data$NAME %in% c("Houston"), 'red', 
#                           ifelse(texas@data$NAME %in% c("Dallas"), 'blue', 
#                                  ifelse(texas@data$NAME %in% c("Austin"), 'green',
#                                         ifelse(texas@data$NAME %in% c("San Antonio"), 'yellow', 'white'))))
...
>>> Adicionar grafico com distribição de doações vs cand vs previa
...
ggplot(aes(x=contb_receipt_dt_month, y=contb_receipt_amt, fill=contb_receipt_amt_range), 
       data = subset(USACampaign, party %in% c("DEM","REP") & contb_receipt_dt_year %in% c("15","16")))+
  geom_bar(stat = 'summary', fun.y = 'sum')+
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
...
```{r echo=FALSE, warning=FALSE}
ggplot(aes(y = contb_receipt_amt, x=contb_receipt_dt_month, color=contb_receipt_amt_range),
...
ggplot(aes(y = contb_receipt_amt, x = contb_receipt_dt, fill = contb_receipt_amt_range), 
       data=subset(USACampaign, cand_nm %in% top3_cand_total & !is.na(contb_receipt_amt)))+
  ...
  facet_wrap(~cand_nm)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position="bottom")+
  ggtitle('Valor total de doações ao longo do tempo, por candidato')


ggplot(aes(x=contb_receipt_dt_month, y=contb_receipt_amt, fill=contb_receipt_amt_range), 
       data = subset(USACampaign, cand_nm %in% top3_cand_total & contb_receipt_dt_year %in% c("15","16")))+
  geom_bar(stat = 'summary', fun.y = 'sum')+
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        legend.position = "bottom")+
  scale_x_date(date_breaks = "4 months", 
...
df_por_candidato <- function(candidato) {
  
  df <- subset(USACampaign, cand_nm == candidato) %>%
    group_by(city) %>%
    summarise(qtde = n(),
...
gerar_grafico_por_candidato <- function(candidato) {
    ggplot(aes(y = vr_total, x = as.factor(city), color = qtde, size = qtde), 
           data = head(df_por_candidato(candidato), 10))+
      geom_point()+
...
for (i in 1:length(top7_cand)) {
    print(gerar_grafico_por_candidato(as.character(top7_cand[i])))
}
...
df.by_party <- subset(USACampaign, party %in% c("DEM","REP")) %>%
                group_by(party, city) %>%
                summarise(qtde = n(),
                          total = sum(contb_receipt_amt),
                          media = mean(contb_receipt_amt),
...
df.by_party <- dcast(df.by_party, city ~ party, value.var = c('total'))

df.by_party$party_most_contr <- ifelse(is.na(df.by_party$DEM), "REP",
                                       ifelse(is.na(df.by_party$REP), "DEM",
                                              ifelse(df.by_party$DEM > df.by_party$REP, "DEM", "REP")))

table(df.by_party$party_most_contr)

#texas@data <- left_join(texas@data, df.by_party, by = c("NAME", "city"))

#texas@data$color <- ifelse(df.by_party[texas@data$NAME,]$party_most_contr == "DEM", 'red', 
#                           ifelse(df.by_party[texas@data$NAME,]$party_most_contr == "REP", 'blue', 'white'))

#plot(texas, border=gray(0.9), col = as.character(texas@data$color))

df.by_party$city = tolower(df.by_party$city)
ia <- map_data("county", "texas")
mid_range <- function(x) mean(range(x))
ia <- left_join(ia, df.by_party, by = c("subregion" = "city"))
ia <- subset(ia, select = -c(REP, DEM))

#ia$party <- df.by_party[ia$subregion,]$party_most_contr

#seats <- plyr::ddply(ia, "subregion", plyr::colwise(mid_range, c("lat", "long")))
ggplot(data = ia, aes(long, lat)) +
  geom_polygon(aes(group = group, fill = party_most_contr), colour = "grey60") 

data(us.cities)
texas_cities <- subset(us.cities, country.etc == 'TX')
texas_cities$name <- gsub(" TX","",texas_cities$name)

ggplot(texas_cities, aes(long, lat)) +
